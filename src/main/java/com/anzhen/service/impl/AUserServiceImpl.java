...
public class AUserServiceImpl extends ServiceImpl<AUserMapper, AUser> implements AUserService {
  ...
  @Resource AUserMapper aUserMapper;
  @Resource ARoleUserService aRoleUserService;
  @Resource ARoleService aRoleService;
  ...
  public UserInfo findInfoById(Long id) {
    ...
    List<Long> roleIds =
        CollectionUtil.isEmpty(aRoleUserList)
            ? List.of()
            : aRoleUserList.stream().map(ARoleUser::getRoleId).toList();
    List<ARole> aRoles =
        CollectionUtil.isEmpty(roleIds) ? List.of() : aRoleService.listByIds(roleIds);
